...
public class Test {
	...
	private static <A> boolean compareTreeIterator(TreeIter<A> it, String steps, A[] results)
	{
		if (it == null) {
			return false;
		}
	...
	private static void testReplaceableTreeWithPerson(
			ReplaceableTree<Person> reptree, PreorderTree<Student> pretree,
			PostorderTree<Professor> posttree)
	{
		...
		test(prefix + "testing iterator of tree after inserting subtrees",
				true, compareTreeIterator(reptree.iterator(), 
						"next next next next next next next next next next next next next next next",
				new Person[] {samples[0],samples[1],samples[2],samples[3],samples[4],samples[5],samples[6],samples[1],samples[0],samples[1],samples[2],samples[3],samples[4],samples[5],samples[6]}));
		
		test(prefix + "contains with valid value after replace", true, compareTreeIterator(reptree.contains(samples[5]),
				"next next next",
				new Person[] {samples[4],samples[5],samples[6]}));
		test(prefix + "contains with invalid value after replace", null, reptree.contains(new Student("blub",0)));
		test(prefix + "search with valid value after replace", true, compareIterator(reptree.search(samples[6]), new Boolean[]{false, true, true}));
		test(prefix + "search with invalid value after replace", null, reptree.search(new Student("blub",0)));
		test(prefix + "search root after replace", true, compareIterator(reptree.search(samples[1]), new Boolean[]{}));	
	}
	...
	private static void testPreorderTreeWithStudent(PreorderTree<Student> pretree)
	{
		...
		test(prefix + "preorder iterator", true, compareTreeIterator(pretree.iterator(),
				...
				new Student[] {samples[3],samples[1],samples[0],samples[2],samples[5],
				samples[4],samples[6],samples[4],samples[5],samples[2],samples[0],
				samples[1],samples[3]}));
		test(prefix + "bounds of iterator", true, compareTreeIterator(pretree.iterator(),
				...
				new Student[] {null,samples[3],samples[1],samples[0],samples[2],samples[5],
				samples[4],samples[6],null,samples[4]}));
		...
		test(prefix + "down of iterator at leaf", true, compareTreeIterator(pretree.iterator(),
				"next next next down next prev",
				new Student[] {samples[3],samples[1],samples[0], null, samples[0], null}));
		test(prefix + "contains with valid value", true, compareTreeIterator(pretree.contains(samples[1]),
				"next next next",
				new Student[] {samples[1], samples[0], samples[2]}));
		test(prefix + "contains with invalid value", null,
				pretree.contains(new Student("Unbekannt", 0)));
		...
		test(prefix + "search with invalid value", null,
				pretree.search(new Student("Unbekannt", 0)));
		test(prefix + "search root", true, compareIterator(pretree.search(samples[3]),
				new Boolean[]{}));
	}
	...
	private static void testPostorderTreeWithProfessor(PostorderTree<Professor> postree)
	{
		...
		test(prefix + "postorder iterator", true, compareTreeIterator(postree.iterator(),
				"next next next next next next next prev prev prev prev prev prev prev",
				new Professor[] {samples[0],samples[2],samples[1],samples[4],samples[6],
				samples[5],samples[3],samples[5],samples[6],samples[4],samples[1],
				samples[2],samples[0]}));
		test(prefix + "bounds of iterator", true, compareTreeIterator(postree.iterator(),
				"prev next next next next next next next next prev",
				new Professor[] {null,samples[0],samples[2],samples[1],samples[4],
				samples[6],samples[5],samples[3],null,samples[5]}));
		test(prefix + "down of iterator", true, compareTreeIterator(postree.iterator(),
				"next next next down next next next",
				new Professor[] {samples[0],samples[2],samples[1],null,samples[0],
				samples[2],samples[1]}));
		test(prefix + "down of iterator at leaf", true, compareTreeIterator(postree.iterator(),
				"down next prev",
				new Professor[] {null, samples[0], null}));
		test(prefix + "contains with valid value", true, compareTreeIterator(postree.contains(samples[1]),
				"next next next",
				new Professor[] {samples[0], samples[2], samples[1]}));
		test(prefix + "contains with invalid value", null,
				postree.contains(new Professor("Unbekannt", "Z")));
		test(prefix + "search with valid value, right tree", true,
		...
		test(prefix + "search with invalid value", null,
				postree.search(new Professor("Unbekannt", "Z")));
		test(prefix + "search root", true,
				compareIterator(postree.search(samples[3]), new Boolean[]{}));
	}
	...
	private static void testInorderTreeWithInteger(InorderTree<Integer> intree)
	{
		...
		test(prefix + "down of iterator", true, compareTreeIterator(intree.iterator(),
				"next next down next next next",
				new Integer[] {1, 2, null, 1, 2, 3}));
		test(prefix + "down of iterator at leaf", true, compareTreeIterator(intree.iterator(),
				"down next prev",
				new Integer[] {null, 1, null}));
		test(prefix + "contains with valid value", true, compareTreeIterator(intree.contains(2),
				"next next next",
				new Integer[] {1, 2, 3}));
		test(prefix + "contains with invalid value", null, intree.contains(9));
		test(prefix + "search with valid value, right tree", true, compareIterator(intree.search(5), new Boolean[]{true, false}));
		test(prefix + "search with valid value, left tree", true, compareIterator(intree.search(3), new Boolean[]{false, true}));
		test(prefix + "search with invalid value", null, intree.search(9));
		test(prefix + "search root", true, compareIterator(intree.search(4), new Boolean[]{}));
	}
	...
	private static void testReplaceableTreeWithString(ReplaceableTree<String> rtree)
	{
		...
		test(prefix + "iterator (inorder)", true, compareTreeIterator(rtree.iterator(),
				"next next next next next next next prev prev prev prev prev prev prev",
				new String[] {"replT_a","replT_b","replT_c","replT_d","replT_e","replT_f","replT_g","replT_f","replT_e","replT_d","replT_c","replT_b","replT_a"}));
		test(prefix + "bounds of iterator", true, compareTreeIterator(rtree.iterator(),
				"prev next next next next next next next next prev",
				new String[] {null,"replT_a","replT_b","replT_c","replT_d","replT_e","replT_f","replT_g" ,null,"replT_f"}));
		test(prefix + "down of iterator", true, compareTreeIterator(rtree.iterator(),
				"next next down next next next",
				new String[] {"replT_a","replT_b",null,"replT_a","replT_b","replT_c"}));
		test(prefix + "down of iterator at leaf", true, compareTreeIterator(rtree.iterator(),
				"down next prev",
				new String[] {null, "replT_a", null}));
		test(prefix + "contains with valid value", true, compareTreeIterator(rtree.contains("replT_b"),
				"next next next",
				new String[] {"replT_a", "replT_b", "replT_c"}));
		test(prefix + "contains with invalid value", null, rtree.contains("blub"));
		test(prefix + "search with valid value, right tree", true, compareIterator(rtree.search("replT_e"), new Boolean[]{true, false}));
		test(prefix + "search with valid value, left tree", true, compareIterator(rtree.search("replT_c"), new Boolean[]{false, true}));
		...
		if (subTree != null)
		{
			...
			test(prefix + "iterator after replacing subtree", true, compareTreeIterator(rtree.iterator(),
					"next next next next next next next prev prev prev prev prev prev prev",
					new String[] {"replT_a","replT_b","replT_c","replT_d","subT_a","subT_b","subT_c","subT_b","subT_a","replT_d","replT_c","replT_b","replT_a"}));
		}
